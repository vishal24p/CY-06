...
def test_connector_behavior_is_offline_and_boundary_safe():
    ...
    with patch("cy06.connector.boto3.Session", return_value=Mock(client=Mock(return_value=sts))) as session:
...
def test_credential_boundary_errors_are_chained(exception):
    with patch("cy06.connector.boto3.Session", side_effect=exception):
        with pytest.raises(ConnectionError, match="credentials unavailable") as raised:
            connect("profile", EXPECTED_ROLE_ARN)
