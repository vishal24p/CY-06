...
export function InventoryTables({ inventory }: { inventory: Inventory }) {
  ...
  return (
    <section className="space-y-5" aria-labelledby="tables-heading">
      <div><p className="font-mono text-xs uppercase tracking-[0.2em] text-cyan-300">Source inventory</p><h2 id="tables-heading" className="mt-1 text-xl font-semibold text-white">IAM tables</h2></div>
...
function DataTable({ title, columns, rows }: { title: string; columns: string[]; rows: string[][] }) {
  return <div className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/60"><div className="border-b border-slate-800 px-5 py-3"><h3 className="font-semibold text-white">{title} <span className="ml-2 font-mono text-xs text-slate-500">{rows.length}</span></h3></div><div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><caption className="sr-only">{title} IAM records</caption><thead className="bg-slate-950/70 text-xs uppercase tracking-wider text-slate-500"><tr>{columns.map((column) => <th key={column} scope="col" className="whitespace-nowrap px-5 py-3">{column}</th>)}</tr></thead><tbody className="divide-y divide-slate-800">{rows.length ? rows.map((row, rowIndex) => <tr key={`${title}-${rowIndex}`} className="hover:bg-slate-800/40">{row.map((cell, cellIndex) => <td key={`${rowIndex}-${cellIndex}`} className="max-w-[360px] whitespace-normal break-all px-5 py-3 align-top font-mono text-xs text-slate-300">{cell || "—"}</td>)}</tr>) : <tr><td colSpan={columns.length} className="px-5 py-5 text-sm text-slate-500">No records in uploaded inventory.</td></tr>}</tbody></table></div></div>;
}
